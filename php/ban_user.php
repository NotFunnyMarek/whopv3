<?php
// php/ban_user.php

// ——————————————————————————————————————————————
// 1) CORS & headers
// ——————————————————————————————————————————————
header("Access-Control-Allow-Origin: http://localhost:3000");
header("Access-Control-Allow-Credentials: true");
header("Access-Control-Allow-Methods: POST, OPTIONS");
header("Access-Control-Allow-Headers: Content-Type");
header("Content-Type: application/json; charset=UTF-8");

// Handle preflight OPTIONS request
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(200);
    exit;
}

// ——————————————————————————————————————————————
// 2) Session & authentication
// ——————————————————————————————————————————————
require_once __DIR__ . '/session_init.php';
$user_id = $_SESSION['user_id'] ?? null;
if (!$user_id) {
    http_response_code(401);
    echo json_encode([
        "status"  => "error",
        "message" => "Unauthorized – not logged in"
    ]);
    exit;
}

// ——————————————————————————————————————————————
// 3) Parse JSON input
// ——————————————————————————————————————————————
$input = json_decode(file_get_contents('php://input'), true);
if (
    !$input ||
    !isset($input['ban_user_id']) ||
    !isset($input['whop_id'])
) {
    http_response_code(400);
    echo json_encode([
        "status"  => "error",
        "message" => "Missing 'ban_user_id' or 'whop_id'"
    ]);
    exit;
}
$banUserId = intval($input['ban_user_id']);
$whopId    = intval($input['whop_id']);

// ——————————————————————————————————————————————
// 4) Database connection
// ——————————————————————————————————————————————
require_once __DIR__ . '/config_login.php';
try {
    $pdo = new PDO(
        "mysql:host={$servername};dbname={$database};charset=utf8mb4",
        $db_username,
        $db_password,
        [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]
    );
} catch (PDOException $e) {
    http_response_code(500);
    echo json_encode([
        "status"  => "error",
        "message" => "Connection failed: " . $e->getMessage()
    ]);
    exit;
}

// ——————————————————————————————————————————————
// 5) Mark all of the banned user's submissions as rejected
// ——————————————————————————————————————————————
try {
    $updateSubs = $pdo->prepare("
        UPDATE submissions AS s
        JOIN campaign    AS c ON s.campaign_id = c.id
        SET s.status = 'rejected',
            s.rejection_reason = 'User Banned',
            s.updated_at = UTC_TIMESTAMP()
        WHERE s.user_id = :ban_user_id
          AND c.whop_id = :whop_id
          AND c.is_active = 1
    ");
    $updateSubs->execute([
        'ban_user_id' => $banUserId,
        'whop_id'     => $whopId
    ]);
} catch (PDOException $e) {
    error_log("ban_user: failed to reject submissions: " . $e->getMessage());
    // Continue even if this fails
}

// ——————————————————————————————————————————————
// 6) Perform ban and remove membership
// ——————————————————————————————————————————————
try {
    // a) Insert or update the ban record in whop_bans
    $stmt = $pdo->prepare("
        INSERT INTO whop_bans (user_id, whop_id, banned_at)
        VALUES (:ban_user_id, :whop_id, UTC_TIMESTAMP())
        ON DUPLICATE KEY UPDATE banned_at = UTC_TIMESTAMP()
    ");
    $stmt->execute([
        'ban_user_id' => $banUserId,
        'whop_id'     => $whopId
    ]);

    // b) Remove any paid membership
    $delPaid = $pdo->prepare("
        DELETE FROM memberships
        WHERE user_id = :ban_user_id
          AND whop_id = :whop_id
    ");
    $delPaid->execute([
        'ban_user_id' => $banUserId,
        'whop_id'     => $whopId
    ]);

    // c) Remove any free membership
    $delFree = $pdo->prepare("
        DELETE FROM whop_members
        WHERE user_id = :ban_user_id
          AND whop_id = :whop_id
    ");
    $delFree->execute([
        'ban_user_id' => $banUserId,
        'whop_id'     => $whopId
    ]);

    echo json_encode([
        "status"  => "success",
        "message" => "User banned and their submissions have been rejected."
    ]);
    exit;
} catch (PDOException $e) {
    http_response_code(500);
    echo json_encode([
        "status"  => "error",
        "message" => "Error banning user: " . $e->getMessage()
    ]);
    exit;
}
